...
TRIAGE_WORKER_PROMPT = (
    ...
    "<examples>\n"
    "<example>\n"
    "Input: {\"id\": 1, \"from\": \"noreply@notion.so\", \"subject\": \"Your Notion invoice\", \"snippet\": \"Invoice #INV-2026-001 for $10.00 has been paid\"}\n"
    "Output: {\"id\": 1, \"transactional\": true}\n"
    "Reason: Invoice with payment confirmation = transactional\n"
    "</example>\n"
    "<example>\n"
    "Input: {\"id\": 2, \"from\": \"no-reply@accounts.google.com\", \"subject\": \"Security alert\", \"snippet\": \"New sign-in to your Google Account from Chrome on Mac\"}\n"
    "Output: {\"id\": 2, \"transactional\": false}\n"
    "Reason: Security notification, no money involved\n"
    "</example>\n"
    "<example>\n"
    "Input: {\"id\": 3, \"from\": \"support@stripe.com\", \"subject\": \"Payment failed\", \"snippet\": \"Your payment of $29.00 for Stripe Atlas failed\"}\n"
    "Output: {\"id\": 3, \"transactional\": true}\n"
    "Reason: Failed payment = real financial event (charge was attempted)\n"
    "</example>\n"
    "</examples>\n\n"
    "Return JSON: {\"decisions\": [{\"id\": <email_id>, \"transactional\": true/false}]}\n"
    "No other text."
)
...
CORRELATION_PROMPT_TEMPLATE = (
    ...
    "If all are distinct real transactions, return: {{\"merges\": []}}\n\n"
    "Transactions:\n{transactions_json}"
)
...
def _call_api_with_retry(client, circuit_breaker=None, **kwargs):
    """Call Anthropic API with retry on rate limit / overload errors."""
    if circuit_breaker and not circuit_breaker.can_call():
        raise CircuitOpenError('Circuit breaker is open — too many consecutive failures')
...
def _verify(client, prompt, rate_limiter, model=None, circuit_breaker=None):
    """
    ...
    verify_model = model or MODEL_VERIFIER
...
def _triage_single_batch(client, emails_data, rate_limiter, circuit_breaker=None):
    """
    ...
    user_msg = f"Emails ({len(emails_data)}):\n{emails_text}"
...
def _process_triage_batch(api_key, emails_data, batch_num, total_batches, rate_limiter, circuit_breaker=None):
    ...
    try:
        decisions = _triage_single_batch(client, emails_data, rate_limiter, circuit_breaker=circuit_breaker)
        ...
        # Fail safe: treat all as transactional, mark as failed for retry
        return [{"id": em["id"], "transactional": True, "_failed": True} for em in emails_data]


def _run_triage_verifier(client, all_decisions, emails_data, rate_limiter, circuit_breaker=None):
    """
    ...
    corrections = _verify(client, prompt, rate_limiter, circuit_breaker=circuit_breaker)
...
def _run_triage_pass(user, api_key, rate_limiter, circuit_breaker=None, pipeline_run=None):
    """
    Pass 1: Triage -- classify emails as transactional or not.
    Worker classifies in batches, Verifier reviews decisions.
    Failed batches are retried once. Results saved independently per batch.
    Returns stats dict.
    """
    ...
    with ThreadPoolExecutor(max_workers=TRIAGE_MAX_CONCURRENT) as executor:
        ...
        for batch_idx, batch_data in enumerate(all_batches):
            ...
            future = executor.submit(
                ...
                api_key, batch_data, batch_num, total_batches, rate_limiter, circuit_breaker,
            )
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Even future.result() failures get fail-safe defaults
                batch_data = all_batches[batch_num - 1]
                all_decisions.extend([
                    {"id": em["id"], "transactional": True, "_failed": True}
                    for em in batch_data
                ])

    # --- Retry failed batches once ---
    failed_email_ids = {d["id"] for d in all_decisions if d.get("_failed")}
    if failed_email_ids:
        failed_batch_data = [e for e in emails_data if e["id"] in failed_email_ids]
        logger.info(f'[Triage] Retrying {len(failed_batch_data)} emails from failed batches')
        try:
            client = anthropic.Anthropic(api_key=api_key)
            retry_decisions = _triage_single_batch(
                client, failed_batch_data, rate_limiter, circuit_breaker
            )
            # Replace fail-safe decisions with real results
            all_decisions = [d for d in all_decisions if d["id"] not in failed_email_ids]
            all_decisions.extend(retry_decisions)
            logger.info(f'[Triage] Retry succeeded for {len(retry_decisions)} emails')
        except Exception as e:
            logger.error(f'[Triage] Retry also failed: {e}')
            # Keep the fail-safe defaults (all transactional), just strip the _failed flag
            all_decisions = [
                {k: v for k, v in d.items() if k != "_failed"}
                for d in all_decisions
            ]

    # Store failed batch info in PipelineRun state if available
    remaining_failed_ids = {d["id"] for d in all_decisions if d.get("_failed")}
    if remaining_failed_ids and pipeline_run:
        try:
            state = dict(pipeline_run.state) if pipeline_run.state else {}
            state['triage_failed_email_ids'] = list(remaining_failed_ids)
            pipeline_run.state = state
            pipeline_run.save(update_fields=['state'])
        except Exception:
            pass  # Don't break the pipeline for state tracking

    # Strip _failed flag from any remaining decisions
    all_decisions = [
        {k: v for k, v in d.items() if k != "_failed"}
        for d in all_decisions
    ]
    ...
    client = anthropic.Anthropic(api_key=api_key)
    corrections = _run_triage_verifier(client, all_decisions, emails_data, rate_limiter, circuit_breaker=circuit_breaker)
...
def _extraction_single_batch(client, user, emails_data, batch_stats, rate_limiter, circuit_breaker=None):
    """
    ...
    response = _call_api_with_retry(
        ...
        tools=[
            *EXTRACTION_TOOLS[:-1],
            {**EXTRACTION_TOOLS[-1], "cache_control": {"type": "ephemeral"}},
        ],
        messages=messages,
    )
...
def _process_extraction_batch(api_key, user, emails_data, batch_num, total_batches, rate_limiter, circuit_breaker=None):
    ...
    try:
        _extraction_single_batch(client, user, emails_data, batch_stats, rate_limiter, circuit_breaker=circuit_breaker)
    except anthropic.RateLimitError:
        logger.warning(
        ...
        logger.error(
            f'[Extraction] Batch {batch_num}/{total_batches} error (thread-{thread_id}): {e}'
        )
        batch_stats["errors"] += 1
        # Do NOT re-raise: other batches should continue processing.
        # Any transactions saved before the error are preserved (tool handlers
        # save to DB immediately). Unprocessed emails remain as triage_passed
        # and will be picked up in the next extraction round.
...
def _run_extraction_verifier(user, client, rate_limiter, circuit_breaker=None):
    """
    ...
    corrections = _verify(client, prompt, rate_limiter, model=MODEL_VERIFIER, circuit_breaker=circuit_breaker)
...
def _run_extraction_pass(user, api_key, rate_limiter, circuit_breaker=None, pipeline_run=None):
    """
    ...
    Each batch saves results independently via tool handlers (DB writes are immediate).
    Returns stats dict.
    """
    ...
    # Extraction has built-in batch isolation + retry: each batch saves results
    # independently via tool handlers (DB writes happen immediately). If a batch
    # fails, its emails stay as triage_passed and are automatically retried in
    # the next round. This gives up to MAX_EXTRACTION_ROUNDS attempts per email.
        ...
        with ThreadPoolExecutor(max_workers=EXTRACTION_MAX_CONCURRENT) as executor:
            ...
            for batch_idx, batch_data in enumerate(all_batches):
                ...
                future = executor.submit(
                    ...
                    api_key, user, batch_data, batch_num, total_batches, rate_limiter, circuit_breaker,
                )
                futures[future] = batch_num
    ...
    client = anthropic.Anthropic(api_key=api_key)
    extraction_corrections = _run_extraction_verifier(user, client, rate_limiter, circuit_breaker=circuit_breaker)
...
def _run_correlation_pass(user, api_key, rate_limiter, circuit_breaker=None):
    """
    ...
    for vendor_name, txs in multi_groups.items():
                            ...
                            "properties": {
                                "merges": {
                                    ...
                                    "items": {
                                        "type": "object",
                                        "properties": {
                                            "keep_id": {"type": ["integer", "null"]},
                                            "delete_id": {"type": "integer"},
                                            "reason": {"type": "string"}
                                        },
                                        "required": ["keep_id", "delete_id", "reason"],
                                        "additionalProperties": False
                                    }
                                }
                            },
        ...
        try:
            result = json.loads(response_text)
            merge_instructions = result.get('merges', [])
        except (json.JSONDecodeError, TypeError):
            logger.warning(
        ...
        verifier_result = _verify(client, verifier_prompt, rate_limiter, model=MODEL_VERIFIER, circuit_breaker=circuit_breaker)
    ...
    if shipping_txs and order_txs:
        ...
        cross_prompt = (
            ...
            "or {\"merges\": []} if no cross-vendor matches.\n\n"
        ...
        try:
            response = _call_api_with_retry(
                client, circuit_breaker=circuit_breaker, model=MODEL_CORRELATION, max_tokens=2048,
            ...
            response_text = _get_response_text(response)
            try:
                cross_result = json.loads(response_text)
                cross_instructions = cross_result.get('merges', [])
            except (json.JSONDecodeError, TypeError):
                cross_instructions = []
...
# ============================================================
# HELPER FUNCTIONS FOR PIPELINE PASSES 5-6
# ============================================================

def _run_bank_correlation(user):
    """Pass 5: Bank correlation (pure Python)."""
    try:
        from ai_agent.services.correlation import correlate_transactions
        return correlate_transactions(user)
    except Exception as e:
        logger.error(f'[Bank-Correlation] Error: {e}')
        return {'bank_matched': 0, 'bank_unmatched': 0, 'bank_total': 0}


def _run_provider_correlation(user):
    """Pass 6: Provider correlation (pure Python)."""
    try:
        from ai_agent.services.correlation import correlate_providers
        return correlate_providers(user)
    except Exception as e:
...
def run_pipeline(user):
    """
    Run the full multi-pass email processing pipeline with checkpoint/resume.
    ...
    Resumes from last checkpoint if a previous run crashed.
    Returns a combined stats dict with pipeline_run_id.
    """
    ...
    if pipeline_run:
        logger.info(f'====== PIPELINE RESUME (run #{pipeline_run.pk}, pass {pipeline_run.current_pass}) ======')
    else:
        pipeline_run = PipelineRun.objects.create(user=user, status=PipelineRun.Status.PENDING)
        logger.info(f'====== PIPELINE START (run #{pipeline_run.pk}) ======')

    rate_limiter = RateLimiter()
    circuit_breaker = CircuitBreaker()
    all_stats = dict(pipeline_run.stats)  # restore partial stats

    PASSES = [
        (1, PipelineRun.Status.TRIAGE, 'triage', lambda: _run_triage_pass(user, api_key, rate_limiter, circuit_breaker=circuit_breaker, pipeline_run=pipeline_run)),
        (2, PipelineRun.Status.EXTRACTION, 'extraction', lambda: _run_extraction_pass(user, api_key, rate_limiter, circuit_breaker=circuit_breaker, pipeline_run=pipeline_run)),
        (3, PipelineRun.Status.MERGE, 'merge', lambda: _run_correlation_pass(user, api_key, rate_limiter, circuit_breaker=circuit_breaker)),
        (4, PipelineRun.Status.COMPUTATION, 'computation', lambda: _run_computation_pass(user)),
        (5, PipelineRun.Status.BANK_CORRELATION, 'bank_correlation', lambda: _run_bank_correlation(user)),
        (6, PipelineRun.Status.PROVIDER_CORRELATION, 'provider_correlation', lambda: _run_provider_correlation(user)),
    ]

    try:
        for pass_num, status, name, pass_fn in PASSES:
            if pipeline_run.current_pass >= pass_num:
                logger.info(f'[Pipeline] Skipping pass {pass_num} ({name}) — already completed')
                continue
            ...
            try:
                pass_stats = pass_fn()
                all_stats.update(pass_stats)
            except Exception as e:
                logger.error(f'[Pipeline] Pass {pass_num} ({name}) failed: {e}')
                raise

            pipeline_run.current_pass = pass_num
            pipeline_run.stats = all_stats
            pipeline_run.save(update_fields=['current_pass', 'stats'])

        pipeline_run.status = PipelineRun.Status.COMPLETE
        pipeline_run.completed_at = timezone.now()
        pipeline_run.save(update_fields=['status', 'completed_at', 'stats'])

    except Exception as e:
        pipeline_run.status = PipelineRun.Status.FAILED
        pipeline_run.error_message = str(e)
        pipeline_run.save(update_fields=['status', 'error_message', 'stats'])
        logger.error(f'====== PIPELINE FAILED (run #{pipeline_run.pk}) ====== {e}')
        all_stats['error'] = str(e)

    all_stats['pipeline_run_id'] = pipeline_run.pk
    logger.info(f'====== PIPELINE DONE (run #{pipeline_run.pk}, status={pipeline_run.status}) ======')
    return all_stats
